...
import { motion, useMotionValue, useSpring, animate, useAnimationFrame, MotionValue } from "framer-motion";
import { cn } from "@/lib/utils";
import { useEffect, useState, useRef } from "react";

interface InteractivePhysicsTextProps {
  text: string;
  className?: string;
  containerRef?: React.RefObject<HTMLDivElement | null>;
}

// Physics state for a single letter
interface PhysicsBody {
  ...
  motionRotate: MotionValue<number>;
}
...
function DraggableLetter({
  ...
  isComplete: boolean;
  anyDropped: boolean;
  onSnapChange: (index: number, snapped: boolean) => void;
  onDragStateChange: (index: number, dragging: boolean) => void;
  onDrop: () => void;
  onHoverChange: (id: string, type: 'solid' | 'ghost' | 'floating' | null) => void;
}) {
  ...
  const updateSnapped = (newVal: boolean) => {
    if (isSnapped !== newVal) {
...
export default function InteractivePhysicsText({
  ...
  className,
  containerRef,
}: InteractivePhysicsTextProps) {
  const items = text.split("");
  const [snappedCount, setSnappedCount] = useState(0);
  const [droppedCount, setDroppedCount] = useState(0);
  const [isHovering, setIsHovering] = useState(false);
  const [hoveredItems, setHoveredItems] = useState<Record<string, 'solid' | 'ghost' | 'floating'>>({});
  
  const isComplete = snappedCount === items.length && droppedCount === items.length;
  const isAnyDropped = droppedCount > 0;
  
  const handleHoverChange = (id: string, type: 'solid' | 'ghost' | 'floating' | null) => {
  ...
  let currentHoverType: 'solid' | 'ghost' | 'floating' | null = null;
  ...
  const layoutOffsetsRef = useRef<{ x: number; y: number }[]>([]);

  // Initialize physics bodies
  const physicsState = useRef<PhysicsBody[]>(
    items.map(() => {
  ...
  const handleSnapChange = (index: number, snapped: boolean) => {
  ...
  const handleDragStateChange = (index: number, dragging: boolean) => {
    const b = physicsState.current[index];
